...
- **Use instead**: surfaces `bg-bg` / `bg-bg-secondary` / `bg-bg-tertiary`; text `text-fg` / `text-fg-secondary` / `text-fg-tertiary` / `text-fg-brand` / `text-fg-error`; borders `border-border` / `border-border-secondary` / `border-border-brand` / `border-border-error`; actions `bg-primary` / `bg-primary-hover` / `bg-primary-active` / `text-primary-fg`; status `bg-{info,success,warning,error}-bg` + matching `-border` / `-fg`; neutral chip `bg-neutral-bg` / `border-neutral-border`. Need a new dark value? Add a flipping token in `packages/tokens` (`:root` light + `[data-theme="dark"]` override), don't reach for a scale.
