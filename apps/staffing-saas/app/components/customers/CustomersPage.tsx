...
const COLUMNS = [
  ...
  { id: "jobs", name: "Số Jobs" },
  { id: "freelancers", name: "Số Freelancers" },
  { id: "status", name: "Trạng thái hợp tác" },
  { id: "verification", name: "Trạng thái xác thực" },
...
function CustomerRow({ customer }: { customer: CustomerRecord }) {
  ...
  return (
    <Table.Row id={customer.id}>
      ...
      <Table.Cell>
        <div className="text-sm text-fg">{NUMBER_FORMATTER.format(customer.totalJobs)}</div>
      </Table.Cell>
      <Table.Cell>
        <div className="text-sm text-fg">
          {NUMBER_FORMATTER.format(customer.totalFreelancers)}
        </div>
      </Table.Cell>
      <Table.Cell>
