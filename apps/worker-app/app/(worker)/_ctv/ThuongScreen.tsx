...
import { Badge, type BadgeColor, Button, Card, CardContent, ProgressBarBase, Tab, TabList, Tabs, toast } from "@mvp-ui/ui";
import {
  EARNINGS,
  MILESTONES,
  money,
...
export function ThuongScreen() {
  ...
  const shown = useMemo(() => {
    if (filter === "processing") return PEOPLE.filter((p) => p.status === "new" || p.status === "matching");
    ...
    <Card className="shadow-lg">
      <CardContent className="p-[18px]">
        <div className="flex items-start justify-between gap-3">
          ...
          <Button color="primary" size="md" iconLeading={Banknote} onClick={() => toast.success("Mở màn rút tiền…")}>
            Rút tiền
          </Button>
        </div>
...
function MiniStat({ label, value, tone }: { label: string; value: string; tone?: "warning" }) {
  return (
    <div className="px-3 py-2.5 text-center">
      ...
      <div className={`mt-0.5 text-[14px] font-semibold ${tone === "warning" ? "text-warning-fg" : "text-fg"}`}>
        {value}
      </div>
    </div>
  );
      ...
      <CardContent className="p-[18px]">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="text-[12px] text-fg-tertiary">{EARNINGS.monthLabel} · càng nhiều, thưởng càng lớn</div>
          </div>
        ...
        <p className="mt-2.5 text-[13px] text-fg-secondary">
          {next ? (
            <>
              Còn <b className="font-semibold text-fg">{next.people - count} người</b> nữa để nhận thêm{" "}
              <b className="font-semibold text-fg-brand">{plusMoney(next.bonus)}</b>
            </>
          ) : (
            <>
              Đã đạt mốc cao nhất tháng này — nhận{" "}
              <b className="font-semibold text-fg-brand">{plusMoney(current ? current.bonus : 0)}</b>
            </>
          )}
        </p>
        ...
        <div className="mt-3.5 flex gap-2 overflow-x-auto pb-0.5 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {MILESTONES.map((m) => {
            ...
            return (
              <div key={m.people} className={`min-w-20 shrink-0 rounded-xl border px-3 py-2.5 ${cls}`}>
...
const STEPS: Array<{ n: number; body: React.ReactNode }> = [
  { n: 1, body: <><b className="font-semibold text-fg">Chia sẻ link / QR</b> — bạn bè đăng ký bằng số điện thoại của bạn.</> },
  { n: 2, body: <>Bạn bè <b className="font-semibold text-fg">đi làm buổi đầu thành công</b> → bạn nhận ngay <b className="font-semibold text-fg">30.000đ</b>.</> },
  { n: 3, body: <>Giới thiệu nhiều trong tháng để nhận thêm <b className="font-semibold text-fg">thưởng vượt mốc</b> (tới 1.600.000đ).</> },
];
    ...
    <li className="flex items-start gap-3 rounded-xl border border-border-secondary bg-bg p-3">
      ...
      <div className="flex shrink-0 flex-col items-end gap-1">
        ...
        {person.status === "counted" ? (
          <span className="text-[13px] font-semibold text-fg-success">{plusMoney(person.bonus)}</span>
