...
export async function downloadPosterPng(ctv: CtvProfile, brandHex: string, filename: string): Promise<void> {
  const { default: QRCodeStyling } = await import("qr-code-styling");
  const W = 1080;
  const H = 1440;
  ...
  const [logo, avatar, qrBlob] = await Promise.all([
    ...
    new QRCodeStyling({ width: 380, height: 380, data: ctv.fullLink, type: "canvas", margin: 0, ...QR_STYLE }).getRawData("png"),
  ]);
  ...
  if (avatar) {
    ...
    ctx.drawImage(avatar, (avatar.width - s) / 2, (avatar.height - s) / 2, s, s, acx - ar, acy - ar, ar * 2, ar * 2);
    ctx.restore();
  }
  ctx.lineWidth = 6;
