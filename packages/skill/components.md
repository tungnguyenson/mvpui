...
| MetricStrip | `MetricStrip` | Compact KPI row — icon chip + value + label inline, separated by dividers | Props-array API (`items: MetricStripItem[]`). Layout is **container-query** based (not viewport): when the strip's own container is `< 32rem` it goes `mobileLayout="stack"` (default, vertical) or `"scroll"` (horizontal swipe); `≥ 32rem` it's a row. This means it lays out correctly inside a fixed-width shell (e.g. a 448px phone frame) on any viewport. Value text `truncate`s as an overflow guard. Smaller footprint than a grid of `MetricCard`. |
...
| HeaderNav | `HeaderNav`, `HeaderNavProps`, `HeaderNavItemDef` | App top-bar navigation. One prop-driven component covers simple / `centered` / secondary row. Pass `items` + `activeHref`; `logo` and `actions` are slots (actions is consumer-owned — search/notif/account). Below `lg` it collapses to a hamburger `Drawer` (mobile bar + `mobileFooter` slot) | Secondary row auto-derives from the active item's `items` (or explicit `subItems`); `secondaryType="buttons"` (default) or `"tabs"`. Active detection is **prefix-match** (`/projects` active on `/projects/active`), unlike SidebarNav's exact match. `current` on an item overrides. Inner content capped at `max-w-7xl`. `hideBorder` drops the bottom border but keeps the row divider when the secondary row shows |
