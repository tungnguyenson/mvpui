...
/* -------------------------------------------------------------------------- */
/*  Default placeholder icon (inline — no external dep)                       */
/* -------------------------------------------------------------------------- */
...
type AvatarSize = keyof typeof SIZE_DATA;
...
function OnlineIndicator({
  ...
  return (
    <span
      className={cn(
        "absolute right-0 bottom-0 rounded-full border-2 border-bg",
        status === "online" ? "bg-fg-success" : "bg-fg-disabled",
        SIZE_DATA[size].indicator,
      )}
    />
  );
}
...
function VerifiedTickIcon({ size, className }: { size: AvatarSize; className?: string }) {
  ...
  return (
    <svg
      ...
      aria-label="Verified"
    >
      {/* Platform blue — no semantic token for verified badge color */}
      <path
...
export const Avatar = ({
  ...
  const mainContent = (() => {
    ...
    if (initials) {
      return <span className={cn("text-fg-tertiary", s.initials)}>{initials}</span>;
    }
    if (PlaceholderIcon) {
      return <PlaceholderIcon className={cn("text-fg-tertiary", s.icon)} />;
    }
    return placeholder ?? <UserIcon className={cn("text-fg-tertiary", s.icon)} />;
  })();

  const decoration = (() => {
    if (status) return <OnlineIndicator size={size} status={status} />;
    if (verified) {
      return (
        <VerifiedTickIcon
          size={size}
