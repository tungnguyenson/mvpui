 ...
 * Path: components/foundations/play-button-icon.tsx
 *
...
export interface PlayButtonIconProps extends HTMLAttributes<HTMLDivElement> {
  isPlaying?: boolean;
}

export const PlayButtonIcon = ({ isPlaying = false, ...props }: PlayButtonIconProps) => (
  <div
    {...props}
    className={cn(
      "flex size-20 items-center justify-center rounded-full bg-white/30 backdrop-blur transition duration-100 ease-linear group-hover:bg-white/50 hover:bg-white/50", // dark-ok
      props.className,
    )}
  >
    {isPlaying
