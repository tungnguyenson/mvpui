 ...
 * Path: components/shared-assets/qr-code.tsx
 *
...
const QRCodeFrameHandle = ({ className, ...props }: HTMLAttributes<HTMLDivElement>) => (
    <div {...props} className={cn("size-3 rounded-tl border-t-2 border-l-2 border-border-brand", className)} />
);

export const GradientScan = ({ className, ...props }: HTMLAttributes<HTMLDivElement>) => (
    <div
        {...props}
        className={cn("absolute bottom-0 h-1/2 w-full border-t border-border-brand bg-primary/10", className)}
        style={{
